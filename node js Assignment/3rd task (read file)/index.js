...
const data = `The Node.js architecture is based on a single-threaded event loop. This means that the Node.js engine can only execute one task at a time. However, the event loop is very efficient at handling asynchronous tasks. When a task is submitted to the event loop, it is added to a queue. The event loop then checks the queue for tasks that are ready to be executed. When a task is ready, the event loop executes it and then removes it from the queue.
